...
def login():
    ...
    if request.method == 'POST':
        ...
        if not user:
            return jsonify({
                'status': 0,
                'msg': f'User {employeeId} not found',
                'code': 'USER_NOT_FOUND'
            })
...
def adminLogin():
    ...
    if request.method == 'POST':
        ...
        if admin and check_password_hash(admin['password'], password):
            ...
            return jsonify({'status':0,"msg":f'Invalid Wallet or password {adminAddress}'})
...
def addEmployee():
    ...
    if request.method == 'POST':
        ...
        emp = {
            ...
            "fname":fname,
            "lname":lname,
            "revenueDeptId":int(revenueDeptId)
        }
        ...
        try:
            ...
            res = mapRevenueDeptIdToEmployee(int(revenueDeptId),employeeId)
